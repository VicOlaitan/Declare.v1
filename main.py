...
def main():
    ...
    ui_font = _typo.body_bold(UI_FONT_SIZE)
    small_font = _typo.body(SMALL_FONT_SIZE)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                ...
                if settings_open and current_screen in ("game", "menu"):
                    settings_open = False
                    audio.play("ui_close")
                    continue
            ...
            if current_screen == "menu":
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    ...
                    if game_settings.shuffle_enabled and human_idx is not None and cp.is_human:
                    ...
                    if clicked_btn:
                        if clicked_btn == 'draw' and game_manager.state == GameState.TURN_START:
                            ...
                            if game_settings.confirm_declare and awaiting != 'confirm_declare':
                                        ...
                                        if rect.collidepoint(mouse_pos) and slot_idx != selected_slot and game_manager.players[human_idx].hand[slot_idx] is not None:
                                            if slot_idx in game_manager.players[human_idx].known_cards:
                                                ...
                                                if card_a and card_b and card_a.rank == card_b.rank:
                                                    ...
                                                    game_manager.execute_self_pair_action(selected_slot, slot_idx)
        ...
        if current_screen == "game" and game_manager is not None:
            ...
            if not cp.is_human and turn_end_timer <= 0:
                ...
                if ai_phase == 'acting' and ai_timer >= max(1.0, game_settings.ai_delay):
                    ...
                    if drawn:
                        ...
                        if action_key == 'play_power':
                            ...
                            game_manager._last_action_type = 'swap'
                            game_manager._last_action_rank = drawn.rank
                            if game_manager.start_reaction_window(drawn.rank, ai_idx):
                                game_manager.reaction_result_callback = lambda: _resume_ai_turn()
                                ai_phase = 'waiting_reaction'
                                ai_timer = 0.0
                                continue
                            ...
                            game_manager._last_action_type = 'discard'
                            game_manager._last_action_rank = drawn.rank
                            if game_manager.start_reaction_window(drawn.rank, ai_idx):
                                game_manager.reaction_result_callback = lambda: _resume_ai_turn()
                                ai_phase = 'waiting_reaction'
                            ...
                            game_manager._last_action_type = 'pair'
                            game_manager._last_action_rank = drawn.rank
                            if game_manager.start_reaction_window(drawn.rank, ai_idx):
                                game_manager.reaction_result_callback = lambda: _resume_ai_turn()
                                ai_phase = 'waiting_reaction'
                ...
                if ai_phase == 'waiting_reaction':
                    if game_manager.state == GameState.REACTION_WINDOW:
                        for ai_p in game_manager.players:
                            if ai_p.is_human or ai_p.seat_index == game_manager.reaction_source_player:
                                continue
